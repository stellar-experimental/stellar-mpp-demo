...
import { DEFAULT_DECIMALS, NETWORK_PASSPHRASE, SOROBAN_RPC_URLS, } from '../../constants.js';
...
export function channel(parameters) {
    const { channel: channelAddress, commitmentKey: commitmentKeyParam, decimals = DEFAULT_DECIMALS, network = 'testnet', rpcUrl, sourceAccount, store, } = parameters;
    ...
    return Method.toServer(ChannelMethod, {
        ...
        async verify({ credential }) {
            ...
            if (!/^[0-9a-f]+$/i.test(signatureHex) || signatureHex.length % 2 !== 0) {
                throw new ChannelVerificationError('Invalid signature: not a valid hex string.', { signature: signatureHex });
            }
            if (signatureHex.length !== 128) {
                throw new ChannelVerificationError(`Invalid signature length: expected 128 hex chars (64 bytes), got ${signatureHex.length}.`, { length: String(signatureHex.length) });
            }
            ...
            if (store) {
                const stored = await store.get(cumulativeKey);
                if (stored && typeof stored === 'object' && 'amount' in stored) {
                    previousCumulative = BigInt(stored.amount);
                }
            }
            // The new cumulative must be >= previous cumulative
            if (commitmentAmount < previousCumulative) {
                throw new ChannelVerificationError(`Commitment amount ${commitmentAmount} is less than previous cumulative ${previousCumulative}.`, {
            ...
            if (commitmentAmount < previousCumulative + requestedAmount) {
                throw new ChannelVerificationError(`Commitment amount ${commitmentAmount} does not cover the requested amount ${requestedAmount} (previous cumulative: ${previousCumulative}).`, {
                    commitmentAmount: commitmentAmount.toString(),
            ...
            if (!rpc.Api.isSimulationSuccess(simResult)) {
                throw new ChannelVerificationError('Failed to simulate prepare_commitment for verification.', {
                    error: 'error' in simResult
...
export async function close(parameters) {
    const { channel: channelAddress, amount, signature, closeKey, network = 'testnet', rpcUrl, } = parameters;
    const resolvedRpcUrl = rpcUrl ?? SOROBAN_RPC_URLS[network];
    const networkPassphrase = NETWORK_PASSPHRASE[network];
    const server = new rpc.Server(resolvedRpcUrl);
    const contract = new Contract(channelAddress);
    ...
    while (txResult.status === 'NOT_FOUND') {
        if (++attempts >= MAX_POLL_ATTEMPTS) {
            throw new ChannelVerificationError(`Transaction not found after ${MAX_POLL_ATTEMPTS} attempts.`, { hash: result.hash });
        }
        await new Promise((r) => setTimeout(r, 1000));
        txResult = await server.getTransaction(result.hash);
    }
    if (txResult.status !== 'SUCCESS') {
        throw new ChannelVerificationError(`Close transaction failed: ${txResult.status}`, { hash: result.hash, status: txResult.status });
    }
    return result.hash;
}
