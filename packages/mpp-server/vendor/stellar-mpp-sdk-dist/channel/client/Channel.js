...
import { NETWORK_PASSPHRASE, SOROBAN_RPC_URLS, } from '../../constants.js';
...
export function channel(parameters) {
    const { commitmentKey: commitmentKeyParam, commitmentSecret, onProgress, rpcUrl, sourceAccount, } = parameters;
    ...
    return Method.toClient(ChannelMethod, {
        ...
        async createCredential({ challenge, context }) {
            ...
            if (!rpc.Api.isSimulationSuccess(simResult)) {
                throw new Error(`Failed to simulate prepare_commitment: ${'error' in simResult ? simResult.error : 'unknown error'}`);
            }
