...
export function charge(parameters) {
    const { currency, decimals = DEFAULT_DECIMALS, feePayer, network = 'testnet', recipient, rpcUrl, store, } = parameters;
            ...
            switch (payload.type) {
                case 'signature': {
                    ...
                    if (txResult.status !== 'SUCCESS') {
                        throw new PaymentVerificationError(`Transaction ${hash} is not successful (status: ${txResult.status}).`, { hash, status: txResult.status });
                    }
                ...
                case 'transaction': {
                    ...
                    const tx = parsed instanceof FeeBumpTransaction
                    ...
                    if (feePayer && !(parsed instanceof FeeBumpTransaction)) {
                        const feePayerKeypair = typeof feePayer === 'string'
                            ? Keypair.fromSecret(feePayer)
                            : feePayer;
                        txToSubmit = TransactionBuilder.buildFeeBumpTransaction(feePayerKeypair, (Number(tx.fee) * 10).toString(), tx, networkPassphrase);
                    ...
                    while (txResult.status === 'NOT_FOUND') {
                        if (++txAttempts >= 60) {
                            throw new PaymentVerificationError(`Transaction not found after ${txAttempts} polling attempts.`, { hash: sendResult.hash });
                        }
                        await new Promise((r) => setTimeout(r, 1000));
                        txResult = await server.getTransaction(sendResult.hash);
                    }
                    if (txResult.status !== 'SUCCESS') {
                        throw new PaymentVerificationError(`Transaction failed on-chain: ${txResult.status}`, { hash: sendResult.hash, status: txResult.status });
                    }
                    return Receipt.from({
                        method: 'stellar',
...
function verifyFromRawOps(tx, expected) {
    ...
    if (!found) {
        throw new PaymentVerificationError('Transaction does not contain a matching SAC transfer invocation.', {
...
function verifySacTransfer(txResult, expected) {
    if (!txResult.envelopeXdr) {
        throw new PaymentVerificationError('Transaction result is missing envelope XDR — cannot verify payment.', {});
    }
    let envelope;
